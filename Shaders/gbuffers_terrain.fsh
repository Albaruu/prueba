...
void main() {
	...
	if (gl_FragData[0].a>0.0){
		...
		if (translucent){
			diffuseSun = diffuseSun+(0.5*-NdotL+0.5+abs(NdotL)*pow(clamp(lightCol.a*dot(normalize(fragpos),sunVec),0.0f,1.0f),60.)*10.);
			skyLight = skyLight*1.25;
		}
		...
		if (diffuseSun > 0.001) {
			...
			if (abs(projectedShadowPosition.x) < 1.0-1.5/shadowMapResolution && abs(projectedShadowPosition.y) < 1.0-1.5/shadowMapResolution){
				...
				float diffthresh = translucent? 0.001 : (facos(diffuseSun)*0.008*sr+0.00008*sr)/(distortFactor*distortFactor);	
		...
		vec3 diffuseLight = direct + (color.a*lmtexcoord.w)*ambient + color.a*torch_lightmap*0.42*vec3(TORCH_R,TORCH_G,TORCH_B)*TORCH_AMOUNT + color.a*0.0013*MIN_LIGHT_AMOUNT;
